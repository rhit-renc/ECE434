...
static int __init ebbgpio_init(void){
   ...
   gpio_export(gpioLED1, false);             // Causes gpio49 to appear in /sys/class/gpio
			                    // the bool argument prevents the direction from being changeda
   ...
   printk(KERN_INFO "GPIO_TEST: Button 1 state is currently: %d\n", gpio_get_value(gpioButton1));
   printk(KERN_INFO "GPIO_TEST: Button 2 state is currently: %d\n", gpio_get_value(gpioButton2));

   // GPIO numbers and IRQ numbers are not the same! This function performs the mapping for us
   irq1Number = gpio_to_irq(gpioButton1);
   printk(KERN_INFO "GPIO_TEST: Button 1 is mapped to IRQ: %d\n", irq1Number);

   irq2Number = gpio_to_irq(gpioButton2);
   printk(KERN_INFO "GPIO_TEST: Button 2 is mapped to IRQ: %d\n", irq2Number);


   // This next call requests an interrupt line
   result1 = request_irq(irq1Number,             // The interrupt number requested
   ...
   // This next call requests an interrupt line
   result2 = request_irq(irq2Number,             // The interrupt number requested
                        (irq_handler_t) ebbgpio_irq_handler, // The pointer to the handler function below
                        IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,   // Interrupt on rising edge (button press, not release)
                        "ebb_gpio_handler",    // Used in /proc/interrupts to identify the owner
                        NULL);                 // The *dev_id for shared interrupt lines, NULL is okay
